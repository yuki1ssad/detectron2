...
class PascalVOCDetectionEvaluator_baseline(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        ret = OrderedDict()
        # 不同IoU值下的mAP（所有类AP的均值）
        mAP = {iou: np.mean(x) for iou, x in aps.items()}
        # current_mAP = {iou: np.mean(x[self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls]) for iou, x in aps.items()}
        # known_mAP = {iou: np.mean(x[:self.prev_intro_cls + self.curr_intro_cls]) for iou, x in aps.items()}
        # unknown_mAP = {iou: np.mean(x[-1]) for iou, x in aps.items()}
        ret["bbox"] = {"AP for all classes": np.mean(list(mAP.values())), "AP50 for all classes": mAP[50]}
        ...
        if self.prev_intro_cls > 0:
            prev_mAP = {iou: np.mean(x[:self.prev_intro_cls]) for iou, x in aps.items()}
            self._logger.info("Prev class AP: " + str(np.mean(list(prev_mAP.values()))))
            self._logger.info("Prev class AP of different IoU[50,100,5]: " + str((list(prev_mAP.values()))))
        ...
        # self._logger.info("Current class AP: " + str(np.mean(list(current_mAP.values()))))
        # self._logger.info("Current class AP of different IoU[50,100,5]: " + str((list(current_mAP.values()))))
        ...
        # self._logger.info("Known AP: " + str(np.mean(list(known_mAP.values()))))
        # self._logger.info("Known AP of different IoU[50,100,5]: " + str((list(known_mAP.values()))))
        ...
        # self._logger.info("Unknown AP: " + str(np.mean(list(unknown_mAP.values()))))
        # self._logger.info("Unknown AP of different IoU[50,100,5]: " + str((list(unknown_mAP.values()))))
