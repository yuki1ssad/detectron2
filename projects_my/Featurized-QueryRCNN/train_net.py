...
def setup(args):
    ...
    """
    cfg = get_cfg()
    add_sparsercnn_config(cfg) # from .defaults import _C , 获取 d2 里 config 文件夹下 defaulta.py 的配置
    cfg.merge_from_file(args.config_file) # config_file 是指定的 yaml 配置文件，通过 merge_from_file 这个函数会将 yaml 文件中指定的超参数对默认值进行覆盖
