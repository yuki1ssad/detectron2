...
class QueryRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        images, images_whwh = self.preprocess_image(batched_inputs)
        # Feature Extraction.
        src = self.backbone(images.tensor) # p2 p3 p4 p5 p6 p7 [bs,256,w_,h_] 每层 w h 不一样
        ...
        proposals, losses = self.rpn_head(images, src, targets) # QGN  proposals: proposal_boxes[100,4], objectness_logits[100], proosal_feats[100,256]
        proposal_boxes = torch.stack([x.proposal_boxes.tensor for x in proposals])
        init_boxes = proposal_boxes.detach()
        proposal_features = torch.stack([x.proposal_feats for x in proposals])
        # Prediction.
