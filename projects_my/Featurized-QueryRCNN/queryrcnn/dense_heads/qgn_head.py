...
class StaRPNHead(nn.Module):
    ...
    def forward(self, features: List[torch.Tensor]):
        ...
        for level, x in enumerate(features):
            t = F.relu(self.conv(x)) # self.conv: Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
            bbox_pred = self.scales[level](self.anchor_deltas(t)) # self.anchor_deltas: Conv2d(256, 4, kernel_size=(1, 1), stride=(1, 1))
            ...
            pred_objectness_logits.append(self.objectness_logits(t)) # self.objectness_logits: Conv2d(256, 1, kernel_size=(1, 1), stride=(1, 1))
            feats.append(self.proposal_feats(t)) # self.proposal_feats: Conv2d(256, 256, kernel_size=(1, 1), stride=(1, 1))
...
class QGN(nn.Module):
    ...
    def forward(
        ...
        pred_objectness_logits, pred_features, pred_anchor_deltas = self.rpn_head(features) # 每个特征层上的维度：pred_objectness_logits[bs, 1, w, h], pred_features[bs, 256, w, h], pred_anchor_deltas[bs, 4, w, h]
        pos_embed = []
        ...
        if self.training:
            ...
            proposals = self.predict_proposals( # 选择 objectness_logits 最高的 100 个 proposals
